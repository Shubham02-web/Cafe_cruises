    ...
    if (user.length > 0) {
      ...
      connection.query(sql, [otp, email], async (err, result) => {
        ...
        res.status(200).json({
          success: true,
          message:
            "Please Do OTP Verification For Log In , otp sended to your email",
        });
      });
    }
  });
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    updatePasswordByMobile(mobileNumber, hashedPassword, (err, result) => {
      if (err) {
...
const resendOtp = (req, res) => {
  ...
  findUserByMobile(mobileNumber, (err, user) => {
    ...
    resendOtpByMobile(mobileNumber, newOtp, (err, result) => {
      if (err) {
      ...
      return res.status(200).json({
        message: "OTP resent successfully. Check your phone.",
        otp: newOtp,
      });
    });
  });
...
export const rentBike = (req, res) => {
  const { bikeRent, bikeName, bikeModel, cityId } = req.body;
...
(bikeImage, bikeRent, bikeName, bikeModel, ownerId , ownerType , cityId) 
VALUES (?, ?, ?, ?, ? , ? , ?)
`;
...
export const bookRide = (req, res) => {
  ...
  connection.query(
    `INSERT INTO RideBookingTemp (fromLocation, toLocation, distance, paymentAmount, otp, userId) VALUES (?, ?, ?, ?, ?, ?)`,
    [from, to, distance, paymentAmount, otp, id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
export const BookBikeAPI = async (req, res) => {
  const { userId, bikeId, bookingDate } = req.body;
...
export const OTPForBikeBook = async (req, res) => {
  ...
  connection.query(
    "SELECT * FROM bikebookings WHERE userId = ?",
    [userId],
    (err, bookings) => {
      if (err) return res.status(500).json({ error: err.message });
      if (!bookings.length)
      ...
      connection.query(
        "SELECT * FROM bikesonrent WHERE id = ?",
        [booking.bikeId],
        (err2, bikeResults) => {
          if (err2) return res.status(500).json({ error: err2.message });
          ...
          connection.query(
            "SELECT id, firstName, lastName, email, mobileNumber FROM users WHERE id = ?",
            [userId],
              ...
              connection.query(
                ...
                (err4, ownerUsers) => {
                  ...
                  const updateAllBookings = (owner) => {
                    ...
                    connection.query(
                      "SELECT bookedBike FROM users WHERE id = ?",
                      [userId],
                      (err5, resultUser) => {
                        let userBookings = [];
                        try {
                          let bookedBikeData = "[]";
                          if (resultUser.length && resultUser[0].bookedBike) {
                        ...
                        connection.query(
                          "UPDATE users SET bookedBike = ? WHERE id = ?",
                          [JSON.stringify(userBookings), userId]
                        );
                      }
                    );

                    const ownerTable = ownerUsers.length ? "users" : "riders";
                    connection.query(
                      `SELECT bookedBike FROM ${ownerTable} WHERE id = ?`,
                      [owner.id],
                      (err6, resultOwner) => {
                        let ownerBookings = [];
                        try {
                          let ownerBookedData = "[]";
                          if (resultOwner.length && resultOwner[0].bookedBike) {
                        ...
                        connection.query(
                          `UPDATE ${ownerTable} SET bookedBike = ? WHERE id = ?`,
                          [JSON.stringify(ownerBookings), owner.id]
                        );
                      }
                    );

                    connection.query(
                      "SELECT bookedBike FROM bikesonrent WHERE id = ?",
                      [bike.id],
                      (err7, bikeResult) => {
                        let bikeBookings = [];
                        try {
                          let bikeBookedData = "[]";
                          if (bikeResult.length && bikeResult[0].bookedBike) {
                        ...
                        connection.query(
                          "UPDATE bikesonrent SET bookedBike = ? WHERE id = ?",
                          [JSON.stringify(bikeBookings), bike.id]
                        );
                      }
                    );
                  ...
                  if (ownerUsers.length) {
                    ...
                    connection.query(
                      "SELECT id, firstName, lastName, email, mobileNumber FROM riders WHERE id = ?",
                      [bike.ownerId],
                      (err5, riderResults) => {
                        if (err5)
...
export const verifyTripOTP = (req, res) => {
  ...
  connection.query(
    ...
    (err, tripResults) => {
      ...
      try {
        existingTravellers = trip.traveller ? JSON.parse(trip.traveller) : [];
      } catch {
        existingTravellers = [];
      }
      ...
      connection.query(
        ...
        (err2, userResults) => {
          ...
          if (!existingTravellers.some((u) => u.id === user.id)) {
            ...
            connection.query(
              "UPDATE upcoming_rides SET traveller = ? WHERE id = ?",
              [JSON.stringify(existingTravellers), upcomingTripId]
            );
          }

          connection.query(
            "SELECT my_trip FROM users WHERE id = ?",
            [userId],
            (err3, myTripResults) => {
              if (err3) return res.status(500).json({ error: err3.message });
              ...
              connection.query(
                "UPDATE users SET my_trip = ? WHERE id = ?",
                [JSON.stringify(myTrips), userId],
                (updateErr) => {
                  if (updateErr)
                  ...
                  connection.query(
                    ...
                    (errUsers, userList) => {
                      ...
                      userList.forEach((usr) => {
                        ...
                        try {
                          trips = usr.my_trip ? JSON.parse(usr.my_trip) : [];
                        } catch {
                          trips = [];
                        }
                      ...
                      return res.status(200).json({
                        message:
                          "Trip joined successfully and all users updated!",
                      });
                    }
                  );
                }
...
export const submitReview = (req, res) => {
  const { userId, targetType, targetId, review } = req.body;

  if (!review || !targetType || !userId || !targetId) {
    return res.status(400).json({ message: "Missing required fields" });
  }

  connection.query(
    "SELECT my_rides, bookedBike, my_trip, my_rewards FROM users WHERE id = ?",
    [userId],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      if (!result.length)
      ...
      try {
        const myRides = userData.my_rides ? JSON.parse(userData.my_rides) : [];
        const bookedBikes = userData.bookedBike
          ? JSON.parse(userData.bookedBike)
          : [];
        const myTrips = userData.my_trip ? JSON.parse(userData.my_trip) : [];
        const myRewards = userData.my_rewards
          ? JSON.parse(userData.my_rewards)
          : [];
        ...
        if (!updated) {
          return res
            .status(404)
            .json({ message: "No matching item found for review" });
        }
